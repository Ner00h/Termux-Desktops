...
# Function to download file (curl first, wget fallback)
download_file() {
    ...
    if [ -e "$1/$2" ]; then
        ...
        if command -v curl >/dev/null 2>&1; then
            curl -L --insecure -o "$1/$2" "$3"
        else
            wget --no-check-certificate -O "$1/$2" "$3"
        fi
...
# Function to download and execute script (curl first)
download_and_execute_script() {
    ...
    if [ -e "/data/local/tmp/start_debian.sh" ]; then
        ...
        if command -v curl >/div/null 2>&1; then
            curl -L --insecure -o "/data/local/tmp/start_debian.sh" \
                "https://raw.githubusercontent.com/LinuxDroidMaster/Termux-Desktops/main/scripts/chroot/debian/start_debian.sh"
        else
            wget --no-check-certificate -O "/data/local/tmp/start_debian.sh" \
                "https://raw.githubusercontent.com/LinuxDroidMaster/Termux-Desktops/main/scripts/chroot/debian/start_debian.sh"
        fi
...
configure_debian_chroot() {
    ...
    case $DE_OPTION in
        ...
        3) install_cinnamon ;;
        4) install_lxde ;;
        *) echo -e "\e[1;31m[!] Invalid option. Exiting...\e[0m"; goodbye ;;
    esac
}
...
download_startxfce4_script() {
    ...
    if [ "$DE_OPTION" -eq 1 ]; then
        if command -v curl >/dev/null 2>&1; then
            curl -L --insecure -o "./startxfce4_chrootDebian.sh" \
                "https://raw.githubusercontent.com/LinuxDroidMaster/Termux-Desktops/main/scripts/chroot/debian/startxfce4_chrootDebian.sh"
        else
            wget --no-check-certificate -O "./startxfce4_chrootDebian.sh" \
                "https://raw.githubusercontent.com/LinuxDroidMaster/Termux-Desktops/main/scripts/chroot/debian/startxfce4_chrootDebian.sh"
        fi

        if [ $? -eq 0 ]; then
            success "startxfce4_chrootDebian.sh downloaded"
        else
            echo -e "\e[1;31m[!] Error downloading startxfce4 script. Exiting...\e[0m"
            goodbye
        fi
    fi
}
...
main() {
    if [ "$(whoami)" != "root" ]; then
        ...
        download_file "$download_dir" "debian12-arm64.tar.gz" \
        "https://github.com/LinuxDroidMaster/Termux-Desktops/releases/download/Debian/debian12-arm64.tar.gz"
